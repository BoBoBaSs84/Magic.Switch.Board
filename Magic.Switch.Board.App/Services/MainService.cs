...
internal sealed class MainService : IMainService
{
	...
	public async Task<ExitCode> Main(string[] args, CancellationToken cancellationToken)
	{
		...
		loggerService.Information($"{nameof(appConfigService)}.{nameof(appConfigService.Write)}: {nameof(success)} = {success}");
		...
		loggerService.Information($"{nameof(deviceConfigService)}.{nameof(deviceConfigService.Write)}: {nameof(success)} = {success}");
